...
Radar::Radar() : IModule(0, Category::VISUAL, "Radar") {
	...
	registerBoolSetting("Show Grid", &grid, grid);
...
void renderEntity(C_Entity* currentEntity, bool isRegularEntity) {
	...
	if (g_Data.isInGame() && g_Data.getLocalPlayer() != nullptr && g_Data.canUseMoveKeys()) {
		...
		if (relPos.x > 0 && relPos.x < size && relPos.y > topPad - cent && relPos.y < topPad + cent) {
			DrawUtils::fillRectangle(vec4_t(relPos.x - pxSize, relPos.y - pxSize, relPos.x + pxSize, relPos.y + pxSize), MC_Color(interfaceColor), pxOpacity);
		}
	}
}
...
void Radar::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (outline) {
		auto interfaceColor = ColorUtil::interfaceColor(1);
		DrawUtils::drawRectangle(vec4_t(-1, topPad - cent, (float)size, topPad + cent), MC_Color(interfaceColor), bgOpacity, 0.75f);
	}
	if (grid)
	{
