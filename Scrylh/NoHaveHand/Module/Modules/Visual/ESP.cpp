...
ESP::ESP() : IModule(0, Category::VISUAL, "Outlines players") {
	registerBoolSetting("ESP Better", &enhance, enhance);
...
void ESP::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (enhance) {
		if (g_Data.canUseMoveKeys()) {
			// Iterate through all entities and draw the ESP
			g_Data.forEachEntity([](C_Entity* ent, bool isRegularEntity) {
				drawESP(ent);
				});
		}
