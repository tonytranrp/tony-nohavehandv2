...
void SettingManager::loadSettings(std::string name, bool create) {
	...
	if (SettingsExists || create) {
		...
		if (SettingsExists) {
			...
			currentSettingsObj["from"] = "NoHaveHand";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			g_Data.getGuiData()->displayClientMessageF("[NoHaveHand] %sSuccessfully %s %s%s%s!", GREEN, !SettingsExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
		}
	}
