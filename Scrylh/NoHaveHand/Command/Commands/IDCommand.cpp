	...
	if (args->at(1) == "pointing") {
		...
		if (pointing->hasEntity()) {
			...
			if (entity != nullptr) {
				...
				clientMessageF("[Nuke] Entity Name: %s", entityName.c_str());
				clientMessageF("[Nuke] Entity ID: %s", entityID.c_str());
			}
		} else {
			clientMessageF("[Nuke] Block Name: %s", blockName);
			clientMessageF("[Nuke] Block ID: %d", blockID);
		}
		return true;
		...
		if (stack->item != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[Nuke] Item Name: %s", name);
			g_Data.getGuiData()->displayClientMessageF("[Nuke] Item ID: %lld", id);
		}
		return true;
	}
	return false;
