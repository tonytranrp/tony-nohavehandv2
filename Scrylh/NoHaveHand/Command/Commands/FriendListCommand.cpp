...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[NoHaveHand] %sCouldn't find player: %s!", RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[NoHaveHand] %s%s is now your friend!", GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[NoHaveHand] %s%s has been removed from your friendlist!", GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[NoHaveHand] %s%s was not in your friendlist!", GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
