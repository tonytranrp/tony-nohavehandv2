...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new AutoLogCommand());
...
void CommandMgr::execute(char* message) {
	if (message != nullptr) {
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sVictory++%s] %sCommand '%s' could not be found!", GRAY, WHITE, RED, cmd.c_str());
	}
}
