...
struct vec3_t {
	...
	// Add the equals function to check if two vec3_t objects are equal
	...
	vec3_t sub(const vec3_t& other) const {
	...
	vec3_t scale(float factor) const {
	...
	vec3_t sub1(const vec3_t& other) const {
		return vec3_t(x - other.x, y - other.y, z - other.z);
	}
...
struct vec3_ti {
	...
	vec3_ti operator-(int scalar) const {
		return vec3_ti(x - scalar, y - scalar, z - scalar);
	}
	...
	vec3_ti& operator+=(const vec3_ti& other) {
		x += other.x;
		y += other.y;
		z += other.z;
		return *this;
	...
	bool operator==(const vec3_ti& other) const {
	...
	float distto(const vec3_ti& other) const {
...
struct vec4_t {
	...
	vec4_t offsetVec4(const vec4_t& original, float offsetX, float offsetY) {
		return vec4_t(original.x + offsetX, original.y + offsetY, original.z + offsetX, original.w + offsetY);
	}
