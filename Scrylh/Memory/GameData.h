...
#include "../NoHaveHand/Config/AccountInformation.h"
...
public:
	...
	vec3_ti GameData::getLookingBlockPos() {
		if (localPlayer != nullptr) {
			auto* pointingStruct = localPlayer->pointingStruct;
			if (pointingStruct != nullptr) {
				return pointingStruct->block;
			}
		}
		return vec3_ti(0, 0, 0);  // Return an invalid block position if not found
	}
