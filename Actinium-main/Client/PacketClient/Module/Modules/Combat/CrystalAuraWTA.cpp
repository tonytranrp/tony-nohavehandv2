...
static uintptr_t HiveRotations3 = Utils::getBase() + 0x8F53B1;
static uintptr_t HiveRotations4 = Utils::getBase() + 0x98AF833C1;
static uintptr_t HiveRotations5 = Utils::getBase() + 0x173ACFA01D; //From Skidders
...
CrystalPlacements CrystalAuraWTA::bestPlaceOnPlane(C_Entity* targ, int yLevel) {
	...
	auto cmpCustom = [targ](const CrystalPlacements& E1, const CrystalPlacements& E2) {
		float distanceE1 = E1.toPlace.dist(*targ->getPos());
		float distanceE2 = E2.toPlace.dist(*targ->getPos());
...
#pragma omp critical
	...
	auto advancedComparator = [target](const CrystalPlacements& place1, const CrystalPlacements& place2) {
		float distance1 = place1.toPlace.dist(*target->getPos());
		float distance2 = place2.toPlace.dist(*target->getPos());
...
vec2_t GetRotations(vec3_t playerEyePos, vec3_t* targetPos) {
	vec3_t delta = targetPos->sub(playerEyePos);
...
void CrystalAuraWTA::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (renderPlacing) {
		for (CrystalInfo& indivCrystal : CJTWDPlaceArr) {
			...
			placeLoc = prevRenderPos.add(placeLoc.sub(prevRenderPos)).mul(0.3f);
